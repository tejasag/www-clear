...
export default function LinkEventRestrictionsModal({
    ...
    const [formData, setFormData] = useState(
        restrictions.reduce(
            (prev, curr) => ({
                ...prev,
                [curr.id]:
                    event.eventRestrictions.filter(
                        (restriction) => restriction.id === curr.id
                    ).length > 0,
            }),
            {}
        )
    ...
    return (
        <Box d="inline" {...props}>
            ...
            >
                <Heading m={2}>Event Restrictions for {event.name}</Heading>
                {restrictions.map((r) => (
                    <Checkbox
                        d="block"
                ...
                <Button
                    ...
                    onClick={async () => {
                        try {
                            ...
                            const setQuery = restrictions.filter(
                                (r) => formData[r.id]
                            );
