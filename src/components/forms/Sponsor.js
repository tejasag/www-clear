...
const uiSchema = {
    ...
    amount: {
        'ui:help':
            'If sponsorship was not cash (for example food) enter a rough estimate of the goods provided',
    },
    perks: {
...
export function UpdateSponsorModal({ sponsor, children, ...props }) {
    const [open, setOpen] = useState(false);
...
export function DeleteSponsorModal({ sponsor, children, ...props }) {
    const [open, setOpen] = useState(false);
    const { data: session } = useSession();
    const fetch = useFetcher(session);
    const [loading, setLoading] = useState(false);
            ...
            >
                ...
                <Button
                    ...
                    onClick={async () => {
                        setLoading(true);
                        try {
                            await fetch(DeleteSponsorMutation, {
                                where: { id: sponsor.id },
                            });
                            await router.replace(router.asPath);
