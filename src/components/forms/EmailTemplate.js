...
const uiSchema = {
    ...
    sendText: {
        'ui:help':
            "If we don't have a number on hand, an email will be sent instead",
    },
    sendLate: {
        'ui:help':
    ...
    sendAfterEvent: {
        'ui:help':
            '**All** Email Templates without this option are barred from being sent if the event has ended',
    },
    sendParent: {
        'ui:help':
            'If selected, this email will only be sent to parents - to send to both, create two Email Templates',
    },
};
...
export function UpdateEmailTemplateModal({
    ...
    function formDataToUpdateInput(formData) {
        ...
        Object.keys(schema.properties).map((key) => {
            if (formData[key] !== emailtemplate[key])
            ...
            >
                ...
                <Button
                    ...
                    onClick={async () => {
                        setLoading(true);
                        try {
                            await fetch(DeleteEmailTemplateMutation, {
                                where: { id: emailtemplate.id },
                            });
                            await router.replace(router.asPath);
