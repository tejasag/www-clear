...
export function UpdateEventGroupModal({ eventgroup, children, ...props }) {
    ...
    const [formData, setFormData] = useState({
        ...
        earlyBirdCutoff: moment(eventgroup.earlyBirdCutoff)
            .utc()
            .format('YYYY-MM-DD'),
        registrationCutoff: moment(eventgroup.registrationCutoff)
            .utc()
            .format('YYYY-MM-DD'),
    });
    const { data: session } = useSession();
    ...
    function formDataToUpdateInput(formData) {
        ...
        Object.keys(schema.properties).map((key) => {
            if (formData[key] !== eventgroup[key])
            ...
            >
                ...
                <Button
                    ...
                    onClick={async () => {
                        setLoading(true);
                        try {
                            await fetch(DeleteEventGroupMutation, {
                                where: { id: eventgroup.id },
                            });
                            await router.replace(router.asPath);
