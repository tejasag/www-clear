...
export function CreateEventModal({ group, children, ...props }) {
    ...
    const [formData, setFormData] = useState({
        ...
        earlyBirdCutoff: moment(group.earlyBirdCutoff)
            .utc()
            .format('YYYY-MM-DD'),
        registrationCutoff: moment(group.registrationCutoff)
            .utc()
            .format('YYYY-MM-DD'),
    });
...
export function UpdateEventModal({ event, children, ...props }) {
    ...
    const [formData, setFormData] = useState({
        ...
        earlyBirdCutoff: moment(event.earlyBirdCutoff)
            .utc()
            .format('YYYY-MM-DD'),
        registrationCutoff: moment(event.registrationCutoff)
            .utc()
            .format('YYYY-MM-DD'),
    });
    const { data: session } = useSession();
            ...
            >
                ...
                <Button
                    ...
                    onClick={async () => {
                        setLoading(true);
                        try {
                            await fetch(DeleteEventMutation, {
                                where: { id: event.id },
                            });
                            await router.replace(router.asPath);
