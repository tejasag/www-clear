...
const uiSchema = {
    type: {
        'ui:help':
            'What kind of Schedule Item is this? (Workshop, Activity, Deadline, Meal, etc)',
    },
    finalized: {
        'ui:help':
            'Non-finalized items will not be displayed on the public event schedule',
    },
    internal: {
        'ui:help':
            'Internal Schedule Items are only shown to volunteers and staff',
    },
};

export function CreateScheduleItemModal({ event, group, children, ...props }) {
    if (event && group)
        throw 'CreateScheduleItemModal must be passed only one of either `event` or `group`';
    if (!event && !group)
        throw 'CreateScheduleItemModal must be passed `event` or `group`';
    const [open, setOpen] = useState(false);
    const [formData, setFormData] =
        useState(/* if you need to set default values, do so here */);
...
export function UpdateScheduleItemModal({ scheduleitem, children, ...props }) {
    ...
    function formDataToUpdateInput(formData) {
        ...
        Object.keys(schema.properties).map((key) => {
            if (formData[key] !== scheduleitem[key])
                ret[key] = { set: formData[key] };
        });
        return ret;
    }
    ...
    return (
        <Box d="inline" {...props}>
            <Button d="inline" onClick={onOpenModal}>
            ...
            >
                ...
                <Button
                    ...
                    onClick={async () => {
                        setLoading(true);
                        try {
                            await fetch(DeleteScheduleItemMutation, {
                                where: { id: scheduleitem.id },
                            });
                            await router.replace(router.asPath);
