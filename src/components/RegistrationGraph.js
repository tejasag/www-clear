...
export default function RegistrationGraph({ event, children, ...props }) {
    ...
    event.tickets
        ...
        .forEach((ticket) => {
            ...
            if (createdAt <= graphStart) {
                ...
                const x = moment
                    .duration(createdAt.diff(graphStart))
                    .as('days');
    ...
    return (
        <InfoBox heading="Registrations" headingSize="xl" {...props}>
            ...
            <Text m={0} ml={0.5} size="sm">
                {event.soldTickets} students;{' '}
                {event.tickets.length - event.soldTickets} staff
            </Text>
            {children}
            {event.soldTickets > 0 && (
                <LineGraph
            ...
            <Button as="a" href={`${event.id}/preRegistrations`} d="inline">
                <Eye />
            </Button>
        </InfoBox>
    );
