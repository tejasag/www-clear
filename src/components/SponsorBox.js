...
import {
    UploadSponsorDarkLogoMutation,
    UploadSponsorLogoMutation,
} from './forms/Sponsor.gql';
    ...
    return (
        <InfoBox
            ...
            heading={
                <>
                    {sponsor.name}
                    <Box d="inline-block">
                        <UpdateSponsorModal sponsor={sponsor} />
        ...
        >
            <Masonry columnsCount={2}>
                    ...
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            if (file.size > MAX_FILE_SIZE) {
                                error(
                                    `You might have a problem uploading files larger than ${Math.floor(
                            ...
                            if (file.size > WARN_FILE_SIZE) {
                                info(
                                    `Your file is uploading, but at ${Math.floor(
                                        file.size / (1024 * 1024)
                                    )}MB, it might take a while.`
                                );
                            } else {
                ...
                <InfoBox heading="Logo (Dark Mode)">
                    <Image
                        src={darkLogoUrl}
                    ...
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            reader.onload = function (el) {
                                setDarkLogoUrl(
                                    el.target.result.darkLogoImageUri
                                );
                            };
                            reader.readAsDataURL(file);
                    ...
                    {!sponsor.darkLogoImageUri ? (
                        <>
                            ...
                            <Button
                                onClick={() =>
                                    darkLogoUploaderRef.current.click()
                                }
                            >
                ...
                <Notes
                    notes={sponsor.notes}
                    updateId={sponsor.id}
                    updateMutation={SetSponsorNotesMutation}
                />
            </Masonry>
            {children}
        </InfoBox>
