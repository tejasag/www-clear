...
import {
    DeleteEventRestrictionModal,
    UpdateEventRestrictionModal,
} from './forms/EventRestriction';
...
function Highlight({ children }) {
    return (
        <Text as="span" bold color="brand.700">
            {children}
        </Text>
    );
}
        ...
        >
            <InfoBox heading="Preview" nested>
                    ...
                    onChange={async (e) => {
                        ...
                        try {
                            ...
                            const result = await fetch(
                                UploadEventRestrictionIconMutation,
                                { where: { id: eventRestriction.id }, file }
                            );
                ...
                <Text bold>
                    {ReactHtmlParser(
                        DOMPurify.sanitize(
                            marked.parse(eventRestriction.title || '')
                        ),
                        { transform }
                    )}
                </Text>
                <Text>
                    {ReactHtmlParser(
                        DOMPurify.sanitize(
                            marked.parse(eventRestriction.details || '')
                        ),
                        { transform }
                    )}
                </Text>
