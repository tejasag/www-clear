...
export default function DaysUntilEvent({ event, ...props }) {
    ...
    const daysUntil = Math.ceil(
        moment.duration(eventStart.diff(now)).as('days')
    );

    if (daysUntil < 0)
        return <DayDisplay text="CodeDay is over. :(" {...props} />;
    if (daysUntil === 0)
        return <DayDisplay redText="It's CodeDay!" {...props} />;
    if (daysUntil === 1)
        return <DayDisplay text="CodeDay is" redText="tomorrow." {...props} />;
    return (
        <DayDisplay
            text="CodeDay is"
            redText={`in ${daysUntil} days.`}
            {...props}
        />
    );
}
