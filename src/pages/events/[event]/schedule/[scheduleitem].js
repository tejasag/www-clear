...
import {
    ...
    UpdateScheduleItemModal,
} from '../../../../components/forms/ScheduleItem';
...
export default function ScheduleItem({ scheduleitem }) {
    ...
    return (
        <Page title={scheduleitem.name}>
            ...
            />
            <Heading>
                {scheduleitem.type ? `${scheduleitem.type}: ` : null}{' '}
                {scheduleitem.name}
...
export async function getServerSideProps({
    ...
    const scheduleitemResults = await fetch(GetScheduleItemQuery, {
        data: { id: itemId },
    });
    return {
        props: {
